(function(a){a.ezLoader={version:"0.1.0",modules:[],events:[],scope:[],scripts:[],stylesheets:[],modulesCount:0,loadedModulesCount:0,options:{baseElement:null,baseUrl:null,paths:{scripts:"scripts/"},noCache:false},init:function(b){if(this.isInit){return false;
...
for(var d=c.length-1;d>=0;d--){var f=c[d].src;if(f.match(/ezLoader(.*)(.js)/)){var b=c[d].getAttribute("data-main"),e=this;if(b){if(!this.isAbsoluteUrl(b)){b=this.getBaseUrl(c[d])+"/"+b+".js";
...
for(var d=c.length-1;d>=0;d--){var e=c[d].src;if(e.match(/ezLoader(.*)(.js)/)){return e.split("/").slice(0,-1).join("/");}}return"";}},getUserAgent:function(){var d=navigator.userAgent;
...
return c;},count:function(b){return b?this.loadedModulesCount:this.modulesCount;},Events:{handlers:[],init:function(){var e=this;var d=function(f){a.ezLoader.events.load=true;
e.fire("load",f);};if(document.readyState=="complete"){d();}else{this.addEvent("load",d);}var c=function(f){a.ezLoader.events.userReady=true;e.fire("userReady",f);
...
document[e]();};}else{document[d]=e;}}}}else{}}},addEvents:function(c,d){for(var b=0;b<c.length;b++){this.addEvent(c[b],d);}}}};a.ezLoader.Events.addEvent("load",function(){a.ezLoader.loadMain();
});})(window);